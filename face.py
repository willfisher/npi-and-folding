...
class Face:
	...
	'''
		Checks if two faces are equal up to cycling their elements. Returns the positive index offset s.t. f1[i] = f2[i + offset] for all i if true,
		else -1.
	'''
	@staticmethod
	def offset_equal(f1, f2):
		...
		for offset in range(len(f1.face)):
			...
			for i in range(len(f1.face)):
				if f1.face[i] != f2.face[(i + offset) % len(f2.face)]:
					worked = False
					break
			if worked:
				return offset
